...
- `MISSING: <tool> (install: <command>)` - list the missing tools to the captain with a one-line purpose each plus the printed install commands, wait for consent (one approval may cover the list), then run `bin/fm-bootstrap.sh install <approved tools...>`.
  ...
  For `no-mistakes`, this also covers an installed version older than 1.31.2.
  ...
  For `quota-axi`, report the missing requirement and do not choose around an unexamined dispatch candidate.
...
- `TANGLE: <remediation>` - the primary checkout is stranded on a feature branch instead of its default branch.
...
- `FLEET_SYNC: <repo>: recovered: <detail>` - the sync self-healed a clean detached HEAD holding no unique commits by re-attaching the default branch and fast-forwarding; no action needed.
...
- `PR_CHECK_MIGRATION: canonical polls rebuilt and armed; resume supervision for this home` or `PR_CHECK_MIGRATION: validated replacement polls armed; resume supervision for this home` - the rebuilt or replacement polls are already armed.
...
- `PR_CHECK_MIGRATION: migration completed safely; resume supervision for this home` - no task poll was rebuilt or quarantined; resume the emitted supervision protocol after finishing the session-start wake handling.
...
- `SECONDMATE_SYNC: secondmate <id>: skipped: <reason>` - the local-HEAD secondmate sync left a live secondmate home on its existing checkout for the printed reason; bootstrap continued, but inspect the reason because the secondmate's tracked instructions, inherited settings, or shared captain preferences may be stale after a primary update.
- `SECONDMATE_LIVENESS: secondmate <id>: skipped: <reason>|respawn failed after <cause>: <reason>` - the liveness sweep could not guarantee that the registered secondmate is running a real agent process; investigate the reason.
