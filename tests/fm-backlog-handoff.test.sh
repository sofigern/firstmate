...
# binary. Skip cleanly when it is absent or too old (matching the backend
# smoke suites), since atomic multi-ID mv needs 0.2.2+.
...
fm_tasks_axi_compatible || { echo "skip: installed tasks-axi is too old for atomic multi-ID mv (needs 0.2.2+)"; exit 0; }
